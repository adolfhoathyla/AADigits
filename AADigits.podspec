#
# Be sure to run `pod lib lint AADigits.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see https://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'AADigits'
  s.swift_versions   = '4.0'
  s.version          = '0.1.0'
  s.summary          = 'AADigits is a component that will help you to control visually and logically the behavior of UITextFields nested in the same logic.'

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!

  s.description      = 'AADigits is a component that will help you to control visually and logically the behavior of UITextFields nested in the same logic. The number of digits is customizable and according to your need.'

  s.homepage         = 'https://github.com/adolfhoathyla/AADigits'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'adolfhoathyla' => 'adolfo.athyla@gmail.com' }
  s.source           = { :git => 'https://github.com/adolfhoathyla/AADigits.git', :tag => s.version.to_s }
  s.social_media_url = 'https://twitter.com/a7hyla'

  s.ios.deployment_target = '8.0'

  s.source_files = 'AADigits/Classes/**/*'
  
  # s.resource_bundles = {
  #   'AADigits' => ['AADigits/Assets/*.png']
  # }

  # s.public_header_files = 'Pod/Classes/**/*.h'
  # s.frameworks = 'UIKit', 'MapKit'
  # s.dependency 'AFNetworking', '~> 2.3'
end
